...
class Hungarian:
    """ the solver for assgiment problem using hungarian method """
    def __init__(self):
        self.rowCover = []
        self.colCover = []
        self.dim = 0
    ...
    def _step_3(self):
        ...
        #count = np.z = np.nonzero(self.matrix == 0)
        #z_col = z[1]
        #z_col_unique = np.unique(z_col)
        count = np.count_nonzero(self.colCover)
        
        #count = z_col_unique.size
        #print 'matrix'
        #print self.matrix, self.marked
        if count >= self.dim:
            return False, 7
    ...
    def make_cost_matrix(c1, c2):
        ...
        for i in range(l1):
            ...
            for j in range(l2):
                ...
                m[i,j] = it_j.size + it_i.size - m_ij.size
